"""
Evaluate Context Recall metric using RAGAS and store in database
"""
import sqlite3
...
# Fetch all responses that need context recall evaluation
query = """
SELECT 
    r.id as response_id,
    q.question_text,
...
WHERE r.id NOT IN (
    SELECT response_id FROM evaluation_metrics WHERE context_recall IS NOT NULL
)
"""
...
print(f"Found {len(rows)} responses to evaluate for context recall")
...
for i, response_id in enumerate(response_ids):
    recall_score = result['context_recall'][i]
    ...
    if existing:
        ...
        cursor.execute("""
            ...
            SET context_recall = ?
            WHERE response_id = ?
        """, (recall_score, response_id))
    else:
        # Insert new record
        cursor.execute("""
            INSERT INTO evaluation_metrics (response_id, context_recall)
            VALUES (?, ?)
        """, (response_id, recall_score))

conn.commit()
print(f"✅ Stored context recall scores for {len(response_ids)} responses")

# Show summary
cursor.execute("""
    SELECT 
        q.document_type,
        AVG(em.context_recall) as avg_recall,
        COUNT(*) as count
...
conn.close()
print("\n✅ Context recall evaluation complete!")
