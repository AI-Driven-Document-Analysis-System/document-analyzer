...
def _extract_key_content(text: str, max_words: int = 3000) -> str:
    ...
    # Split by multiple paragraph delimiters (handles \n\n, \n, or \r\n)
    ...
    # If we got very few paragraphs, split by single newlines instead
    if len(paragraphs) < 3:
        paragraphs = [p.strip() for p in normalized_text.split('\n') if p.strip()]
    
    # If still no good paragraphs, split into sentences as last resort
    if len(paragraphs) < 2:
        paragraphs = [s.strip() for s in re.split(r'[.!?]+', text) if len(s.strip()) > 50]
    
    if not paragraphs:
        logger.warning("Could not extract paragraphs from document, returning original text")
        return text
    ...
    if not scored_paragraphs:
        logger.warning("No paragraphs met selection criteria, returning original text")
        return text
    ...
    # Fallback: if we still have no content, just take the highest-scored paragraphs
    if not selected_content:
        logger.warning("Selection resulted in empty content, using top-scored paragraphs")
        for score, paragraph, para_words in scored_paragraphs[:5]:
            selected_content.append(paragraph)
            total_words += para_words
            if total_words >= max_words * 0.5:
                break
    
    result = '\n\n'.join(selected_content)
    logger.info(f"Extracted key content: {total_words} words from {len(words)} original words ({len(selected_content)} paragraphs selected)")
    
    return result if result.strip() else text  # Return original if extraction failed
