...
#         if word.startswith('##'):
#             if current_entity and current_entity.get('entity_type') == entity_type:
...
#             else:
#                 # Start new entity if types don't match
#                 if current_entity:
...
#     medical_meaningful = {
#         'condition_patterns': [r'\b\w*diabetes\w*\b', r'\b\w*hypertension\w*\b', r'\b\w*cancer\w*\b', r'\b\w*infection\w*\b'],
#         'medication_patterns': [r'\b\w*(cillin|mycin|statin|pril|formin)\w*\b', r'\bmetformin\b', r'\blisinopril\b', r'\baspirin\b'],
#         'procedure_patterns': [r'\b\w*(surgery|therapy|examination|test)\w*\b', r'\bechocardiogram\b', r'\bmri\b', r'\bct scan\b'],
#         'anatomy_patterns': [r'\bheart\b', r'\bliver\b', r'\bkidney\b', r'\bbrain\b', r'\blung\b'],
#         'person_patterns': [r'\b(?:Dr|Doctor|Mr|Mrs|Ms)\.?\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', r'\b[A-Z][a-z]+\s+[A-Z][a-z]+\b']
#     }
    
#     legal_meaningful = {
#         'party_patterns': [r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b', r'\b\w+\s+(?:Inc|LLC|Corp|Ltd)\.?\b'],
#         'legal_terms': [r'\bcontract\b', r'\bagreement\b', r'\bliability\b', r'\bdamages\b', r'\bbreach\b'],
#         'court_terms': [r'\bcourt\b', r'\bjudge\b', r'\bjury\b', r'\btrial\b', r'\bappeal\b'],
#         'person_patterns': [r'\b(?:Judge|Attorney|Mr|Mrs|Ms)\.?\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', r'\b[A-Z][a-z]+\s+[A-Z][a-z]+\b']
#     }
...
#             if entity_type.lower() in ['disease', 'medication', 'treatment', 'anatomy', 'procedure']:
#                 is_meaningful = True
...
#                 confidence > 0.7 and
#                 entity_type.lower() in ['person', 'organization', 'location', 'misc']):
#                 is_meaningful = True
...
#                 abs(len(word_lower) - len(existing_lower)) <= 2):
...
#     formatted_output = "EXTRACTED ENTITIES\n"
#     formatted_output += "═" * 17 + "\n\n"
...
#             if entity_list:
#                 formatted_output += f"▌{entity_type}:\n"
...
#             if len(defendant_name.split()) >= 1:  # At least one name part
...
# def integrate_ner_with_summary(ner_results: str, summary_text: str, domain: str) -> str:
#     """Integrate NER results with the generated summary in a coherent way"""
#     if not summary_text.strip():
#         return ner_results
    
#     if "No significant entities identified" in ner_results or "No meaningful entities found" in ner_results:
#         return summary_text
    
#     # Create integrated output with professional styling
#     integrated_output = ""
    
#     # Add the summary first with professional header
#     if domain == "medical":
#         integrated_output += "EXECUTIVE SUMMARY\n"
#         integrated_output += "═" * 17 + "\n\n"
#     elif domain == "legal":
#         integrated_output += "CASE OVERVIEW\n"
#         integrated_output += "═" * 13 + "\n\n"
#     else:
#         integrated_output += "DOCUMENT SUMMARY\n"
#         integrated_output += "═" * 16 + "\n\n"
    
#     integrated_output += summary_text + "\n\n"
    
#     # Add NER results as supporting details
#     integrated_output += ner_results
    
#     # For domain-specific summaries, add a brief insight connecting the two
#     if domain in ["medical", "legal"]:
#         integrated_output += "\n┌─ KEY INSIGHTS ─┐\n"
        
#         if domain == "medical":
#             integrated_output += "The extracted entities provide structured access to clinical information, supporting medical decision-making and care coordination.\n"
#         elif domain == "legal":
#             integrated_output += "The identified entities facilitate quick reference to critical legal parties, terms, and procedural elements.\n"
...
#             # Store NER results for later integration
#             result += formatted_ner + "\n\n"
...
#             result += "═" * 17 + "\n\n"
#             result += f"Entity extraction unavailable: {str(ner_error)}\n\n"
#     else:
#         print(f"Domain '{domain}' not medical or legal, skipping NER")
#         result += f"DOCUMENT ANALYSIS\n"
#         result += "═" * 17 + "\n\n"
#         result += f"Document Type: {domain.title()}\n"
#         result += f"Specialized entity extraction not available for {domain} documents.\n\n"
...
#         if is_domain_specific:
#             # For domain-specific summaries, get NER analysis first
#             domain_analysis = perform_domain_analysis_with_ner(text, domain)
            
#             if summary_generated and summary_text.strip():
#                 # Get just the NER results from domain analysis
#                 ner_section_start = domain_analysis.find("NAMED ENTITY RECOGNITION RESULTS")
#                 if ner_section_start != -1:
#                     ner_results = domain_analysis[ner_section_start:]
#                     key_info = domain_analysis[:ner_section_start]
                    
#                     # Create integrated output
#                     result = key_info
#                     result += integrate_ner_with_summary(ner_results, summary_text, domain)
#                 else:
#                     # Fallback if NER section not found
#                     result = domain_analysis
#                     result += "SUMMARY\n"
#                     result += "=" * 7 + "\n\n"
#                     result += summary_text.strip()
#             else:
#                 # No summary generated, just return domain analysis
#                 result = domain_analysis
#                 result += "SUMMARY\n"
#                 result += "=" * 7 + "\n\n"
#                 result += "Summary generation failed. Please try again or check your API configuration.\n"
#         else:
#             # For brief and detailed summaries, just return the summary with professional styling
#             if summary_generated and summary_text.strip():
...
#                 result += "═" * 17 + "\n\n"
#                 result += "Summary generation encountered technical difficulties. Please verify API configuration and try again.\n"
...
#     print("=" * 60)
#     print("TESTING DOMAIN-SPECIFIC SUMMARY WITH IMPROVED NER")
#     print("=" * 60)
